...
var track_tracker_service = require("../modules/service/track_tracker_service");
...
router.get('/get_song/:playlist', function(req, res){
    ...
    track_tracker_service.
        get_currently_playing_info(req.params.playlist.toLowerCase(), function(next_song)  {
            if(next_song) {
                console.log("Song found: ", next_song);
                res.json({success: true, song: next_song});
            }
            else {
                console.log("No song found: ", next_song);
                res.json({success: false});
            }
    });
});

router.get('/pause/:playlist', function(req, res){
    track_tracker_service.pause_playlist(req.params.playlist.toLowerCase());
    res.json({
        success: true
    });
});

router.get('/resume/:playlist', function(req, res){
    track_tracker_service.resume_playlist(req.params.playlist.toLowerCase());
    res.json({
        success: true
    });
